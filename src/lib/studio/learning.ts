...
async function hybridSearch(
  ...
  // Prisma.raw is safe here: queryEmbedding is a number[] generated by our own embedding model
  const vectorLiteral = Prisma.raw(`'[${queryEmbedding.map(Number).join(',')}]'::vector`);
  const toolCondition = tool != null ? Prisma.sql`AND "tool" = ${tool}` : Prisma.empty;
  ...
  const results = await db.$queryRaw<Array<{
    ...
    FROM "StudioFeedback"
    WHERE "rating" = ${rating} AND "embedding" IS NOT NULL ${toolCondition}
