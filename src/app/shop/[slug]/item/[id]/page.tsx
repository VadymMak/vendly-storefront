...
export async function generateMetadata({ params }: ItemPageProps) {
  ...
  if (!item) return { title: 'Produkt nenájdený' };
        ...
        <div>
          ...
          {!item.isAvailable && (
            <div className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-600">
              Tento produkt momentálne nie je dostupný.
            </div>
          )}

          {item.isAvailable && item.price !== null && (
            <div className="mt-8">
              <AddToCartButton item={item} scheme={scheme} />
            </div>
          )}
          ...
          {store.settings.whatsapp && (
            <a
              href={`https://wa.me/${store.settings.whatsapp}?text=${encodeURIComponent(`Dobrý deň, mám záujem o: ${item.name}`)}`}
              target="_blank"
