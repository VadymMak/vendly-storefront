...
import { getStoreBySlug, getStoreByDomain, getStoreItem, getRelatedItems } from '@/lib/shop-queries';
...
export async function generateMetadata({ params }: ItemPageProps) {
  ...
  return {
    ...
    openGraph: {
      title: item.name,
      description: item.description || item.name,
      images: item.images.length > 0 ? [{ url: item.images[0] }] : undefined,
    },
  };
}

function ProductJsonLd({ item, store, slug }: { item: ShopItem; store: { name: string }; slug: string }) {
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Product',
    name: item.name,
    description: item.description || undefined,
    image: item.images.length > 0 ? item.images : undefined,
    ...
    offers: item.price !== null ? {
      ...
      url: `https://vendshop.shop/shop/${slug}/item/${item.id}`,
    } : undefined,
  };
...
export default async function ItemPage({ params }: ItemPageProps) {
  ...
  const [scheme, currencySymbol, t, relatedItems] = await Promise.all([
    ...
    getRelatedItems(store.id, item.id, item.category),
  ]);

  return (
    <>
      <ProductJsonLd item={item} store={store} slug={slug} />

      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {/* Breadcrumbs */}
        <nav className={`mb-6 flex items-center gap-1.5 text-sm ${scheme.textMuted}`}>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div>
            ...
            {item.category && (
              <Link
                href={`/?category=${encodeURIComponent(item.category)}`}
                className={`mt-2 inline-block rounded-full px-3 py-1 text-xs font-medium transition-colors ${scheme.bgCard} ${scheme.textMuted} hover:opacity-80`}
              >
                {item.category}
              </Link>
            )}
