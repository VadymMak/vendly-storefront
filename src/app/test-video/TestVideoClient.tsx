                  ...
                  {uploading ? (
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-[var(--color-text-dim)]">JPG, PNG, WEBP, RAW (CR2, NEF, DNG…) · max 50 MB</p>
                      </div>
                    </>
                  )}
                </div>
              )}
              <input ref={fileInputRef} type="file" accept="image/*,.cr2,.cr3,.crw,.nef,.nrw,.arw,.srf,.sr2,.dng,.orf,.raf,.rw2,.srw,.pef,.raw" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadFile(f); e.target.value = ''; }} />
            </section>
          )}
