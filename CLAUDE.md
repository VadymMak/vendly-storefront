...
4. **Коммит сразу после каждого изменения кода** — не ждать запроса от пользователя
5. После задачи — обновить `vendly-storefront-plan-v1.md` (отметить ✅)
6. **Обновлять план после каждого хода** — если были изменения кода, обновить план в том же ходе

### Brain (база знаний — multi-ai-chat)
- **В начале сессии**: вызвать `build_context_for_query` или `search_session_memory` чтобы проверить есть ли релевантные паттерны/баги из прошлых сессий
- **После каждого бага/фикса**: сохранить в brain через `save_session_summary` — причина, фикс, урок
- **После архитектурных решений**: сохранить почему выбрано именно это решение
- Формат сохранения: root cause → fix → lesson learned
