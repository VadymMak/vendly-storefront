import Link from 'next/link';
import type { ShopData, ColorSchemeTokens } from '@/lib/types';
import type { ShopFrontMessages } from '@/lib/shop-i18n';

interface ShopFooterProps {
  store: ShopData;
  scheme: ColorSchemeTokens;
  t: ShopFrontMessages;
}

export default function ShopFooter({ store, scheme, t }: ShopFooterProps) {
  const year = new Date().getFullYear();
  const showBranding = store.ownerPlan === 'FREE' || store.ownerPlan === 'STARTER';
  const s = store.settings;

  return (
    <footer className={`${scheme.footerBg} ${scheme.footerText}`}>
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        {/* Footer grid — 4 columns on desktop */}
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {/* Col 1: Brand */}
          <div>
            <p className="text-xl font-extrabold tracking-tight text-white">{store.name}</p>
            {store.description && (
              <p className="mt-3 text-sm leading-relaxed opacity-60">
                {store.description}
              </p>
            )}
            {/* Social icons */}
            <div className="mt-5 flex items-center gap-3">
              {s.whatsapp && (
                <a
                  href={`https://wa.me/${s.whatsapp}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 transition-all hover:bg-green-500 hover:text-white"
                  aria-label="WhatsApp"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/>
                  </svg>
                </a>
              )}
              {s.instagram && (
                <a
                  href={`https://instagram.com/${s.instagram}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 transition-all hover:bg-gradient-to-br hover:from-purple-500 hover:via-pink-500 hover:to-orange-400 hover:text-white"
                  aria-label="Instagram"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
                  </svg>
                </a>
              )}
              {s.facebook && (
                <a
                  href={`https://facebook.com/${s.facebook}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 transition-all hover:bg-blue-600 hover:text-white"
                  aria-label="Facebook"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                </a>
              )}
            </div>
          </div>

          {/* Col 2: Navigation */}
          <div>
            <h4 className="text-[11px] font-bold uppercase tracking-[0.15em] opacity-50">
              {t.catalog}
            </h4>
            <nav className="mt-4 flex flex-col gap-2.5">
              <a href="#products" className="text-sm opacity-70 transition-opacity hover:opacity-100">{t.navProducts}</a>
              <a href="#reviews" className="text-sm opacity-70 transition-opacity hover:opacity-100">{t.navReviews}</a>
              {s.aboutText && (
                <a href="#about" className="text-sm opacity-70 transition-opacity hover:opacity-100">{t.navAbout}</a>
              )}
              <a href="#contact" className="text-sm opacity-70 transition-opacity hover:opacity-100">{t.navContact}</a>
            </nav>
          </div>

          {/* Col 3: Contact */}
          <div>
            <h4 className="text-[11px] font-bold uppercase tracking-[0.15em] opacity-50">
              {t.contact}
            </h4>
            <div className="mt-4 flex flex-col gap-2.5">
              {s.phone && (
                <a href={`tel:${s.phone}`} className="text-sm opacity-70 transition-opacity hover:opacity-100">
                  {s.phone}
                </a>
              )}
              {s.address && (
                <p className="text-sm opacity-70">{s.address}</p>
              )}
              {s.openingHours && (
                <p className="text-sm opacity-70">{s.openingHours}</p>
              )}
            </div>
          </div>

          {/* Col 4: Legal / Info */}
          <div>
            <h4 className="text-[11px] font-bold uppercase tracking-[0.15em] opacity-50">
              Info
            </h4>
            <div className="mt-4 flex flex-col gap-2.5">
              {s.deliveryInfo && (
                <p className="text-sm opacity-70">{s.deliveryInfo}</p>
              )}
              {showBranding && (
                <p className="text-sm opacity-70">
                  {t.createdOn}{' '}
                  <Link
                    href="https://vendshop.shop"
                    className="font-semibold text-green-400 transition-colors hover:text-green-300"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    VendShop
                  </Link>
                </p>
              )}
            </div>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="mt-10 flex flex-col items-center gap-2 border-t border-white/10 pt-6 sm:flex-row sm:justify-between">
          <p className="text-xs opacity-40">&copy; {year} {store.name}</p>
          {showBranding && (
            <p className="text-xs opacity-40">
              Powered by{' '}
              <Link
                href="https://vendshop.shop"
                className="font-semibold text-green-500 transition-colors hover:text-green-400"
                target="_blank"
                rel="noopener noreferrer"
              >
                VendShop
              </Link>
            </p>
          )}
        </div>
      </div>
    </footer>
  );
}
